...
async function getEDTInfo(date) {
    try {
        const weekNumber = await getWeekNumber(date);
        const yearNumber = await getYearNumber(date)
        const trainProg = "BUT1"
        const group = "3B"
        const response = await axios.get(`https://flopedt.iut-blagnac.fr/fr/api/fetch/scheduledcourses/?week=${weekNumber}&year=${yearNumber}&dept=INFO&train_prog=${trainProg}&group=${group}&lineage=true`)
        //let responseData = []
        ...
        let responseData = []
        responseData = response.data.filter(cour => cour.day === dayInitial.ref)
        ...
        await responseData.forEach(cour => {
            console.log(`Module: ${cour.course.module.name}, Start time: ${formatTime(cour.start_time)}`);
        });
...
async function getFreeRoom(date) {
    const weekNumber = await getWeekNumber(date);
    const yearNumber = await getYearNumber(date);
...
async function getYearNumber(date) {
